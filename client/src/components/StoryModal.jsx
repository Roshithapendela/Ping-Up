      ...
      if (file.type.startsWith("video")) {
        ...
        video.onloadedmetadata = () => {
          ...
          if (video.duration > MAX_VIDEO_DURATION) {
            toast.error("Video duration can't exceed 1min");
            setMedia(null);
            setPreviewUrl(null);
          } else {
            setMedia(file);
    ...
    <div className="fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <button
          ...
          disabled={!(text || media)}
          className={`mt-4 w-full inline-flex items-center justify-center gap-2 py-3 px-4 rounded-lg text-white bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 active:scale-95 transition shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-300 ${
            !(text || media) ? "opacity-50 cursor-not-allowed" : ""
          }`}
        >
          <Sparkle size={18} />
          Create Story
