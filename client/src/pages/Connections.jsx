      ...
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='mb-8 flex flex-wrap gap-6'>
          {dataArray.map((item,index)=>(
            <div key={index} className='flex flex-col items-center justify-center gap-1 border h-20 w-40 border-gray-200 bg-white shadow roundedd-md'>
        ...
        <div className='flex flex-wrap gap-6 mt-6'>
          {dataArray.find((item)=>item.label === currentTab).value.map((user)=>(
            <div className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md' key={user._id}>
              <img src={user.profile_picture} alt="" className='rounded-12 w-12 h-12 shadow-md mx-auto' />
              <div className='flex-1'>
                <p className='tex-slate-700 font-medium'>{user.full_name}</p>
                ...
                <div className='flex max-sm:flex-col gap-2 mt-4'>
                  ...
                  }
                  {
                    currentTab === 'Following' &&(
                      <button className='w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200  active:scale-95 transition text-black cursor-pointer'>Unfollow</button>
                    )
                  }
                  {
                    currentTab === 'Pending' &&(
                      <button className='w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200  active:scale-95 transition text-black cursor-pointer'>Accept</button>
                    )
                  }
                  {
                    currentTab === 'Connections' &&(
                      <button className='w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200 text-slate-800 active:scale-95 transition cursor-pointer flex items-center justify-center gap-1' onClick={()=>navigate(`/messages/${user._id}`)}><MessageSquare className='w-4 h-4'/>Message</button>
                    )
                  }
